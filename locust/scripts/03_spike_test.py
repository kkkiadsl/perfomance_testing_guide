...
부하 단계:
  0~ 30초 :  50명 (안정적인 기준선 — 한계 이하 구간)
  30~ 40초: 300명 (스파이크! 10초 만에 6배 급증, 한계 250명 초과)
  40~ 80초: 300명 (스파이크 유지 — 서버가 버티는지 관찰)
  80~100초:  50명 (급감 — 회복되는지 관찰)
  100~130초: 50명 (회복 후 안정 확인)
  130~140초:  0명 (종료)
...
class SpikeTestShape(LoadTestShape):
    ...
    stages = [
        (30,  50,  10),   # 0~30초   : 기준선 50명 (한계 이하, 안정)
        (40,  300, 300),  # 30~40초  : 스파이크 (10초 만에 300명, 한계 초과)
        (80,  300, 1),    # 40~80초  : 스파이크 유지
        (100, 50,  100),  # 80~100초 : 급감
        (130, 50,  1),    # 100~130초: 회복 후 안정 확인
        (140, 0,   50),   # 종료
    ]
